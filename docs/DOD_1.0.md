...
**Release train:** v1.2.33 baseline
...
OwnMesh v1.2.33 is a stable release for the product surface admitted by the
...
local or authenticated control-plane route. Registry `completeness_claim` is
**true** because there are no admitted-but-unimplemented surfaces or
surface-specific evidence waivers. The separate release-evidence receipt stays
false while external platform, packaging, ChatGPT automation, and independent
...
| 3 | D1/DO/Worker auto provision | **done** | Wrangler configuration, migrations, Durable Object binding, and guided resource provisioning are shipped; the user still supplies and owns the Cloudflare account. v1.2.33 adds the tenant OperationRoom binding, the v3 SQLite migration, the retention cron trigger, and migration 0022 (bootstrap seed, five redundant hot-index drops, quota-probe and cutover tables). |
| 4 | OAuth ChatGPT Personal Plugin | **partial** | DCR, bounded CIMD validation, authorization-code/PKCE, RFC 9207 issuer responses, rotating refresh, encrypted 60-second exact-retry convergence receipts, passkey owner login, and exact callbacks are implemented. Expired, mismatched, and advanced-family refresh reuse remains fail-closed. v1.2.33 adds budget admission with read_only/auth_only degraded modes, quota-aware readiness, and structured 503 + Retry-After semantics so quota exhaustion degrades explicitly instead of killing OAuth. A manual live ChatGPT compatibility receipt exists; reproducible automated external E10 evidence remains open. |
...
| 9 | External CLI interoperability | **done** | External CLIs use the same generic exact-`program`/`args` command, process, PTY, and session model as every executable. OwnMesh ships no coding-agent Profile registry, vendor adapter, authentication probe, normalized vendor event model, or native vendor-session resume claim. ADR 0018 defines the process authorization boundary. |
...
- Publication also depends on a downloaded, checksum-verified Linux x64
  ...
  command, generic session, restart/recovery, and two-Agent transfer suites.
...
- `scripts/check_release_quality.py` checks the publish graph, fail-closed
  ...
  v1.2.33 baseline the unsupported CLI count and surface-specific evidence
