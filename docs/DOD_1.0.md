...
| 13 | Central telemetry default disabled | **done** | Default-off invariant is tested; update/diagnostic feature depth remains W-§14. |
...
| 17 | Security tests, fuzz, audit, SBOM, signed update | **partial** | CI/security are blocking and SBOM fallback is forbidden. External review is W-EXT-SEC; signing is W-SIGN. |
...
|---|---|---|
| W-SIGN | Missing native platform signing/notarization and possibly portable key | Checksums are not signatures. No key means degraded pre-release, never a normal signed-release claim. |
...
| W-§14 | Update/rollback/doctor/support-bundle depth | Privacy defaults are done; update/diagnostics completeness is not. |
