...
- Status: Accepted (updated for v1.2.13 review and #114 remediator)
- Date: 2026-08-16 (updated 2026-08-19 for install remediating OwnMesh-generated drop-ins)
- Deciders: OwnMesh runtime maintainers

## #114 update (install remediates OwnMesh-generated leftovers)

`ownmesh doctor` already disclosed an effective unit that forces a user
namespace and told the operator to re-run `ownmesh service install`. Install
was a no-op when the recorded executable and unit path matched, so a leftover
`10-ownmesh-workspaces.conf` (`# Generated by OwnMesh` plus `ReadWritePaths=`)
survived and kept mapping host uids to 65534.

Install is now the remediator for **OwnMesh-generated** leftovers in the
unit-adjacent `{unit}.d/` directory only. Operator-written drop-ins are not
deleted. If the effective unit still forces a user namespace after that
pass, install fails closed and names the remaining files. The shipped unit
also rate-limits restart storms (`StartLimitBurst=5` /
