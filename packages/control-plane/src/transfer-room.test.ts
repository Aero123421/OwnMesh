...
test("TransferRoom forwards one opaque chunk and persists only contiguous ACK metadata", async () => {
  ...
  const chunk = frame("chunk", { sequence: 0, offset: 0, length: 3, ciphertext_base64: "AQID", chunk_sha256: digest });
  ...
  assert.equal(saved.length, 1); assert.equal(saved[0].contiguous_ack_offset, 3); assert.equal(JSON.stringify(saved[0]).includes("AQID"), false);
...
test("TransferRoom drops in-flight bytes on disconnect and rejects duplicate/gap/fence substitution", async () => {
  ...
  assert.equal((await room.handle(src, frame("chunk", { sequence: 1, offset: 0, length: 1, ciphertext_base64: "AQ==", chunk_sha256: digest }))).error, "non_contiguous_or_busy");
  assert.equal((await room.handle(src, frame("chunk", { sequence: 0, offset: 0, length: 1, ciphertext_base64: "AQ==", chunk_sha256: digest, extra: true }))).error, "bad_chunk");
  assert.equal((await room.handle(src, frame("chunk", { sequence: 0, offset: 0, length: 1, ciphertext_base64: "AQ==", chunk_sha256: digest }))).ok, true);
