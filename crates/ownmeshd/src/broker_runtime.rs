//! Fail-closed loading of the native Linux privileged-broker installation.
//!
...
/// The only local broker inputs the unprivileged daemon is allowed to use.
#[derive(Clone)]
...
fn verify_root_hash(path: &Path, mode: u32, expected_hash: &str) -> Result<(), String> {
    ...
    let bytes = read_regular_owned_mode(path, 0, mode)?;
